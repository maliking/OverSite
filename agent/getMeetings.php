...
$monthMeetings = "SELECT CONCAT(firstName, ',', lastName) as title , DATE_FORMAT(meeting, '%Y-%m-%dT%H:%i:%s') as start, ADDTIME(meeting, '00:15:00') as end, DATE_FORMAT(meeting, '%Y-%m-%dT%H:%i:%s') as id  FROM BuyerInfo WHERE MONTH(registeredDate) >=:registeredDate";
